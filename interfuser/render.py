...
def add_rect(img, loc, ori, box, value, pixels_per_meter, max_distance, color):
    ...
    left_up = np.around(left_up).astype(np.int64)
    left_down = np.around(left_down).astype(np.int64)
    right_down = np.around(right_down).astype(np.int64)
    right_up = np.around(right_up).astype(np.int64)
...
def render_self_car(loc, ori, box, pixels_per_meter=5, max_distance=18):
    img_size = max_distance * pixels_per_meter * 2
...
def render(det_data, pixels_per_meter=5, max_distance=18, t=0):
    ...
    for poi in box_ids:
        ...
        loc_x = center_x + det_data[i, j, 1] + t * det_data[i, j, 6] * ori[0]
        loc_y = center_y + det_data[i, j, 2] - t * det_data[i, j, 6] * ori[1]
...
def render_waypoints(waypoints, pixels_per_meter=5, max_distance=18):
    img_size = max_distance * pixels_per_meter * 2
    ...
    for i in range(len(waypoints)):
        ...
        new_loc = tuple(new_loc.astype(np.int64))
